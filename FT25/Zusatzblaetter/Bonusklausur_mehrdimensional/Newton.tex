...
Gegeben sei die Funktion $f:\R ^2\rightarrow \R $ durch 
$$f(x,y)=x^3-3xy^2.$$
Gesucht sind die station\"aren Punkte dieser Funktion. 
\begin{abc}
\item Geben Sie an, welche Bedingung ein station\"arer Punkt $\vec x\in\R^2$ erf\"ullen muss. 
\item Um eine N\"aherung f\"ur einen solchen Punkt zu berechnen, soll das zweidimensionale Newton-Verfahren angewendet werden. Auf welche Funktion wird das Newton-Verfahren angewendet?\\
...
\item Das Newton-Verfahren wird auf die Funktion $\vec F(x,y)=\nabla f(x,y)$ angewendet. Die Ableitung dieser Funktion ist
\begin{align*}
&\vec F'(x,y)\Bigl(=H_f(x,y)\Bigr)\\
=&\begin{pmatrix}
6x &  -6y\\
-6y & -6x 
\end{pmatrix}.
