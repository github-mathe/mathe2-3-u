...
$$F(s):=\int\limits_0^\infty\EH{-st}\cdot\sqrt{t} \, \mathrm{d} t.$$
\textbf{ Hinweise}:
\begin{itemize}
\item Substituieren Sie $u=\sqrt t$. 
\item Integrieren Sie partiell. 
\item Es gilt $\left(\int_0^{\infty} \EH{-su^2}\, \text du\right)^2  = \int_0^{\infty}\int_0^{\infty} \EH{-s(x^2+y^2)} \text dx\,\text dy.$ \\Dieses Integral k\"onnen Sie l\"osen, indem Sie Polarkoordinaten einführen.
