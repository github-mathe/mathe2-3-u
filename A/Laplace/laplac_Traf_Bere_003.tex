...
\begin{align*}
&&\cL\{f_4(t)\} =& \int\limits_0^\pi\EH{-st}\sin t \d t =\int\limits_0^\pi\Imag(\EH{-st+it}) \d t\\
&&=&\Imag\left(\dfrac{\EH{-st+it}}{-s+i}\right)\Big|_0^\pi=\Imag\left(\dfrac{(-s-i)\EH{-st+it}}{s^2+1}\right)\Big|_0^\pi\\
&&= & \dfrac{\EH{-st}}{s^2+1}\cdot\Imag((-s-i)(\cos t+i\sin t))\Big|_0^\pi=\dfrac{\EH{-st}}{s^2+1}\cdot(-\cos t-s\sin t))\Big|_0^\pi \\
\Rightarrow&&  =  & \frac {\EH{-s\pi}+1}{1+s^2}
\end{align*}
...
{
\textbf{ a)} $\frac As - \frac{2A\EH{-st_0}}{s^2+2s}$, 
\textbf{ b)} $\frac{A(\EH{-as}-\EH{-bs}}{s}$, 
\textbf{ c)} $\frac{1-\EH{-3s}}{s^2}$,
\textbf{ d)} $\frac{1+\EH{-s\pi}}{1+s^2}$
}
