...
$$I=\int\limits_B 2y\mathrm{d}(x,y).$$
\end{abc}
}
\Loesung{
\begin{abc}
...
\begin{align*}
I=&\int\limits_{x=\pi/4}^{5\pi/4}\int\limits_{y=\cos(x)}^{\sin(x)}2y\mathrm{d} y\mathrm{d} x
= \int\limits_{x=\pi/4}^{5\pi/4}\Bigl.y^2\Bigr|_{y=\cos(x)}^{\sin(x)}\mathrm{d} x\\
=& \int\limits_{x=\pi/4}^{5\pi/4}(\sin^2(x)-\cos^2(x))\mathrm{d} x
= \int\limits_{x=\pi/4}^{5\pi/4}(-\cos(2x))\mathrm{d} x\\
