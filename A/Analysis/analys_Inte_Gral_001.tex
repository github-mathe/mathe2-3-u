...
\begin{multicols}{2}
\begin{iii}
\item $\int x\cdot \sin x \,\d x$,
\item $\int \sin^2(x) \,\d x$,
%  \int\frac{x^2+1}{\sqrt x}\,\d x
\item $\int x^2\EH{1-x} \,\d x$,
\item $\int \frac{x}{\cos^2 x}\,\d x$,
\item $\int\limits_{0}^{\pi/4} \frac x{\cos^2 x}\,\d x$
\end{iii}
\end{multicols}
\item Berechnen Sie folgende Integrale mittels einer geeigneten Substitution: 
\begin{multicols}{2}
\begin{iii}
\item $\int\limits_1^2 \frac{3x^2+7}{x^3+7x-2}\,\d x$,
\item $\int\limits_\pi^{3\pi/2}x^2\cos(x^3+2)\,\d x$,
\item $\int\limits_1^2 \frac1x \EH{1+\ln x}\,\d x $,
\item $\int\limits_{1/4}^1 \EH{\sqrt x}\,\d x,$
\item $\int\cosh^2 x \sinh x \,\d x$
