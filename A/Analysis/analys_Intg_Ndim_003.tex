...
$$A=\int\limits_B \d y \d x = \int\limits_0^1\int\limits_{x-2}^{2-x}\d y \d
x=\int \limits_0^1(4-2x)\d x = 4-1=3.$$
\item Der Schwerpunkt $(x_s,\, y_s)^\top$ ergibt sich zu 
\begin{align*}
x_s=&\frac 1 A \cdot \int\limits_B x \d y \d x
 = \frac 13 \int\limits_0^1 x\int\limits_{x-2}^{2-x}\d y \d x\\
