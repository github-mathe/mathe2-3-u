...
$$\begin{pmatrix}x\\y\\z\end{pmatrix}=\begin{pmatrix}r\cos\varphi\\y\\r\sin\varphi\end{pmatrix}, \, \mathrm{d}
(x,y,z)=r\mathrm{d}(r,\varphi,y).$$
...
\begin{align*}
I:=&\int\limits_B
(x^2+y+z^2)\mathrm{d}(x,y,z)=\int\limits_{\varphi=0}^{2\pi}\int\limits_{r=0}^{3} \int\limits_{y=y_-}^{y_+}
(r^2\cos^2\varphi + y+r^2\sin^2\varphi)r\mathrm{d} y\mathrm{d} r \mathrm{d} \varphi\\
=&\int\limits_{\varphi=0}^{2\pi}\int\limits_{r=0}^{3} \left(
r^3(y_+-y_-)+r \frac{y_+^2-y_-^2}2\right)\mathrm{d} r\mathrm{d}\varphi\\
=&\int\limits_{\varphi=0}^{2\pi}\int\limits_{r=0}^{3} 
r^3\sqrt{9-r^2}\mathrm{d} r\mathrm{d}\varphi\\
=&2\pi \left( \left.-r^2\frac{(9-r^2)^{3/2}}3\right|_{r=0}^3 + \int\limits_0^3
2r\frac{(9-r^2)^{3/2}}3\mathrm{d} r \right)\\
=&2\pi \cdot \frac 23\cdot \int\limits_0^3 r(9-r^2)^{3/2}\mathrm{d} r
...
\begin{align*}
\mathrm{d}(x,y,z) =& \left|\det\begin{pmatrix}
...
=& \frac {r^2}{2} \cos\psi>0 \text{(wegen}-\frac{\pi}{2} \leq \psi\leq \frac{\pi}{2}).
\end{align*}
Damit ergibt sich 
\begin{align*}
